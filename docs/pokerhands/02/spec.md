...
- **Metadata store (DynamoDB)**: A DynamoDB table stores job/file metadata per user. Suggested key design: partition key `userId` (Cognito sub), sort key `jobId` (e.g. UUID or requestId). Attributes can include: upload S3 key, transcoded S3 key (after completion), status (e.g. `pending`, `completed`, `failed`), original filename, created at, completed at. The "list my files" API queries by `userId` (and optionally filters by status); presigned download uses the transcoded key stored on the item. Terraform provisions the table (and any GSIs if needed for sort-by-date or status).
...
- A DynamoDB table holds job/file metadata (userId, jobId, upload key, transcoded key, status, timestamps). The list API is driven by querying this table by `userId`; this supports pagination, sort-by-date (via GSI if desired), and consistent metadata without S3 list limits.
